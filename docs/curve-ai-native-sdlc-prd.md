...
| Field        | Value                                                                      |
| ------------ | -------------------------------------------------------------------------- |
| Status       | Remediation in progress; D-001 decided; D-003 local shared-network and private-EKS connectivity direction owner-approved pending exact-head merge; implementation blocked by remaining package prerequisites |
| Owner        | X3M                                                                        |
...
| D-003 | Approve development/staging/production topology, trust zones, residency, persistence, backups, secrets, observability, and HA. | The original `LOCAL_ONLY` evidence remains in [ADR-003](technical/adr-003-runtime-topology.md) (live Temporal topology, pins, evidence, and rollback). Federico's 2026-08-20 direction is recorded in [D-003 private-platform connectivity amendment](technical/d003-private-platform-connectivity-amendment.md) (shared `dev_env`, private EKS/VPC/VPN direction, authenticated non-local Temporal clients, security boundary, revised M0-S3 proof): no Curve-specific local or initial-R1 `NetworkPolicy`; dedicated `curve` namespace by default; Kubernetes `ClusterIP`; internal VPN-only Temporal UI ingress; X3M workload identity and Secrets Manager. Exact-head approval and merge make the amendment effective. Detailed Helm values, database/certificate placement, HA, backup/restore, RPO/RTO, cost, and operational ownership remain deployment-package inputs. | Federico Ocampo for connectivity/trust direction; Platform Operations for environment activation | Local M0-S3 resumes after exact-head amendment merge and context rematerialization; before every staging/production activation | Synthetic local data only until package gates pass; private connectivity direction grants no deployment, protected-data, or SLA claim. | OWNER-APPROVED DIRECTION; exact-head merge pending; activation details OPEN |
