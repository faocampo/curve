...
| Field        | Value                                                                      |
| ------------ | -------------------------------------------------------------------------- |
| Status       | Remediation in progress; D-001 and D-003 `LOCAL_ONLY` decided; implementation blocked by remaining package prerequisites |
| Owner        | X3M                                                                        |
...
| D-003 | Approve development/staging/production topology, trust zones, residency, persistence, backups, secrets, observability, and HA. | `LOCAL_ONLY` is decided in [ADR-003](technical/adr-003-runtime-topology.md) (approved Temporal pins, two-network Compose overlay, worker boundary, evidence, and rollback): approved head `7826f403...`, merge `097016f...`, Temporal Python SDK 1.31.0, `curve-control` and `curve-data` internal networks, synthetic-only data, and M0-S3 executable proof. P0-06A/P0-06B are superseded standalone gates. Staging/production region, Kubernetes placement, HA, persistence, TLS/auth, backup/restore, RPO/RTO, cost, and operational ownership remain open. | Federico Ocampo for `LOCAL_ONLY`; Platform Operations for non-local scopes | Local M0-S3 may proceed after its package prerequisites; before every staging/production M0/M4 activation | Local decision only; no staging/production data, deployment, or SLA claim. | DECIDED for `LOCAL_ONLY`; non-local scopes OPEN |
