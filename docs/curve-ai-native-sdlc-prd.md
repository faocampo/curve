...
| Field        | Value                                                                      |
| ------------ | -------------------------------------------------------------------------- |
| Status       | Remediation in progress; D-001 decided; implementation blocked by remaining package prerequisites |
| Owner        | X3M                                                                        |
...
| ------- | ---- | ------- |
| 0.8 | 2026-08-15 | Recorded owner-approved D-001 foundation, licensing, repository-boundary, exact-head, and review-cadence decisions; allocated additive migration, disabled-state, and rollback implementation proof to M0-01; retained merge/base pinning and all other package gates. |
...
| -- | ----------------- | -------------------------------------- | ----- | ------------ | ---------------------------------- | ------ |
| D-001 | Pin the Plane community-edition commit and complete a reuse-versus-build inventory for work items, pages, estimates, relationships, Gantt, APIs, webhooks, auth, and UI conventions. | Federico Ocampo approved [ADR-001](technical/adr-001-plane-upstream-foundation.md) on 2026-08-15 against content digest `sha256:0c780a0264dcc1a301ee412dfce18c3c50453436679c8d4a55729052bdcdc488`. Official Plane upstream is Curve's updateable baseline; Curve code is implemented/released from the public Plane fork; governance/contracts remain in the separate Curve repository. `origin/preview` and `upstream/preview` are pinned at `1c8a60f858d8472aa56e29994ec1c7926da2c6ce`; approved candidate `d380678912e9b46805ef852d2e05411f1fea6d8b` passed frontend checks/build, 516 backend tests, and local smoke. Community/commercial reuse, AGPL/notices/corresponding-source obligations, ownership, both exact-head dispositions, and review triggers are accepted. Merge/resulting-base pinning remain before M0-01 dispatch; M0-01 owns additive migration, disabled-state, and rollback proof. | Federico Ocampo, CTO at X3M | Review at every Plane foundation upgrade, material licensing change, repository-boundary change, or support-owner change | Keep implementation blocked until the approved Plane candidate is merged/pinned and the consuming package's other gates pass. | DECIDED |
