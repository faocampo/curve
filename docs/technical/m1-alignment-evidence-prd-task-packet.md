...
| Status | `PARTIALLY_IMPLEMENTED / CHILD_PACKET_GATED`; M1-00A (minimal Product core) and M1-01A (Initiative domain/API) are accepted locally, while each remaining child requires its own contracts, dependencies, and applicable material decisions |
| Unmaterialized-child state | `PREPARED_NOT_DISPATCHABLE`; this planning document does not authorize coding-agent dispatch for any remaining child |
| Version | 1.7 |
| Date | 2026-08-30 |
| Product | Curve |
...
| --- | --- | --- |
| Manual-first | Initiative, manual Idea Brief, manual PRD versions/diff, research skip/partial disposition, Gate 1 | M0 dependencies and child-specific M1 contracts; metadata-only synthetic evidence may be used before protected storage activation; manual Idea Brief/PRD body persistence additionally waits for `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract) |
...
These blockers are decision inputs, not selected solutions:

| Blocker | Contradiction to resolve | Consuming children | Fail-closed effect |
| --- | --- | --- | --- |
| `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract) | The manual-first lane requires durable Idea Brief and PRD bodies, while the current ArtifactVersion contract requires a non-null object reference owned by M0-04 (protected-object storage), which remains gated by D-009 (retention and erasure decision). | M1-02A (manual Idea Brief), M1-05A (manual research dossier), M1-06A/B (manual PRD domain/UI), and M1-07 (Gate 1) when it binds those bodies | Contracts and metadata-only work may continue; no coding packet may persist a manual body until an approved representation, classification, lifecycle, and migration contract is versioned. |
| `B-NO-MODEL-BUDGET-01` (explicit no-model and zero-budget representation) | Deterministic/manual planning needs to record that no model route and no paid budget are authorized, while current ExecutionPlan references are non-null. | M3-04 (deterministic execution-plan domain/validator) and M3-05 (Gate 2) | M1 manual work remains available; deterministic planning persistence waits for a versioned sentinel/value-object or approved nullability contract. |
...
| M1-01B (Initiative shell) | Implement the [dedicated Initiative-shell packet](m1-01b-initiative-shell-implementation-task-packet.md) (typed M1-01A client, Curve route/navigation, list/detail/create/actions, three-person validation, responsive/accessibility states, exact paths/commands, blockers, and rollback) | M1-01A (Initiative domain/API); approved [screen contract](ux-m1-01b-initiative-shell.md) (Curve-first flow, state matrix, responsive and accessibility acceptance); B-CODING-TOOLS-01 (local coding-tool execution profile); ordered S -> E1 -> E2 -> C -> P publication; B-CODING-AUTHORITY-01 (trusted human authority and attempt lease) before mutation | Typed-client, browser, keyboard, screen-reader, responsive multi-row, create-reset, badge/avatar, conflict/retry, empty/loading, disabled-state, role-visibility, full Plane CI, same-head CodeQL, and manual UX acceptance |
| M1-02A (manual Idea Brief) | `IDEA_BRIEF` Artifact and immutable/manual draft versions with attributed structured edits for problem, users, outcomes, assumptions, contradictions, blockers, and unknowns | M1-01A (Initiative domain/API); `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract); no model/provider gate | Schema, attribution, concurrent edit, canonical digest, diff, validation, browser, and accessibility tests |
| M1-02B (model refinement) | Generate/regenerate a new Idea Brief version without overwriting human edits | M1-02A (manual Idea Brief); M0-S9C (Model Gateway routing/failover); D-004 (model-gateway decision), D-005 (model/provider data-policy decision), D-014 (budget-policy decision) | Prompt/model/policy/budget pins, no silent fallback, provenance, cancellation, redaction, diff, and failure tests |
...
| M1-03B (Onyx adapter) | Live initiating-user Onyx search and source-access recheck | M1-03A (KnowledgeProvider contract); M0-S9B (external provider transport/administration); D-002 (Onyx delegated-identity decision) and approved deployed OpenAPI/profile; D-009 (retention and erasure decision) for persisted bodies | Two-user ACL, expiry/revocation, durable-wait reauthorization, source deletion, rate limit, outage, redaction, and credential-leakage tests |
| M1-04A (evidence metadata) | Manually attributable EvidenceItem metadata, immutable EvidenceSnapshot and ordered members, AccessEnvelope metadata, citations, claim links, freshness, and authorized UI; provider-derived metadata is a separately activated path | M1-01A (Initiative domain/API) for the manual path; M1-03A (KnowledgeProvider contract) only before provider-derived metadata; synthetic metadata only | Digest/order, claim trace, ACL propagation, classification, material flag, inaccessible approver, and cross-workspace tests |
...
| M1-05A (research lifecycle) | Optional `RESEARCH` Activity state, required/optional question classification, explicit skip, partial coverage, cancellation, and manual dossier version | M1-01A (Initiative domain/API), M1-02A (manual Idea Brief), M1-04A (evidence metadata), and `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract) before dossier-body persistence; no provider/model gate for skip/state work | Skip, mandatory-question blocker, partial/failed coverage, state, audit, cancellation, and immutable dossier tests |
| M1-05B (provider/model research) | Bounded asynchronous research using only approved destinations and budgets | M1-05A (research lifecycle), M1-03B (Onyx adapter), M1-04B (protected evidence bodies), M0-S9B (external provider transport), M0-S9C (Model Gateway); D-004 (model-gateway decision), D-005 (model/provider data-policy decision), D-014 (budget-policy decision) | Budget reserve/exhaustion/release, citations, fact/inference split, provider outage, cancellation, provenance, and no-claim-without-source tests |
| M1-06A (PRD contracts/domain) | `PRD` Artifact, immutable versions, completeness rules, evidence snapshot, parent/supersession, structural diff, and submit command | M1-02A (manual Idea Brief), M1-04A (evidence metadata), and `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract) | Schema, canonical digest, immutable submitted body, diff, completeness, concurrency, source-access, and supersession tests |
...
| M1-06C (PRD generation) | Generate a new PRD version from exact Idea Brief/research/evidence versions | M1-06A (PRD contracts/domain), M0-S9C (Model Gateway routing/failover); D-004 (model-gateway decision), D-005 (model/provider data-policy decision), D-014 (budget-policy decision) | Exact input/model/prompt/policy/budget provenance, redaction, cancellation, deterministic structural validation, and no-overwrite tests |
...
| M1-01B (Initiative shell) | `IMPLEMENTATION_DEFINITION_PREPARED / MACHINE_BLOCKED / NO_DISPATCH` | The [M1-01B implementation task packet](m1-01b-initiative-shell-implementation-task-packet.md) (approved UI scope, exact Plane `9f9bb14...` targets, typed API behavior, tests, commands, policy envelope, ordered publication, and rollback) binds the approved [M1-01B experience contract](ux-m1-01b-initiative-shell.md) (clickable Curve-first flow, state matrix, accessibility criteria, task-based review, and exact-commit evidence). Close B-CODING-TOOLS-01 (local coding-tool execution profile), publish S -> E1 -> E2 -> C -> P (ordered normative/evidence/catalog/registry revisions), and separately close B-CODING-AUTHORITY-01 (trusted human authority and attempt lease) before Plane mutation. |
...
| M1-03B (live Onyx) | `BLOCKED_MATERIAL_DECISION` | M0-S9B (external provider transport/administration), D-002 (Onyx delegated-identity decision), deployed OpenAPI/version, two-user proof, and external-read authorization |
| M1-04A (evidence metadata) | `PREPARED` | M1-01A (Initiative domain/API); evidence/envelope/snapshot contracts; exact dispatch values; M1-03A (KnowledgeProvider contract) only for provider-derived metadata |
...
| M1-05A (research lifecycle/skip) | `PARTIALLY_PREPARED / BODY_CONTRACT_BLOCKED` | Activity/state contracts and upstream manual-lane dependencies may proceed; `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract) blocks dossier-body persistence |
| M1-05B (provider/model research) | `BLOCKED_MATERIAL_DECISIONS` | M0-S9B (external provider transport), M0-S9C (Model Gateway), D-002 (Onyx delegated-identity decision), D-004 (model-gateway decision), D-005 (model/provider data-policy decision), D-009 (retention and erasure decision), and D-014 (budget-policy decision) plus their applicable provider/storage proofs |
| M1-06A/B (manual PRD) | `BLOCKED_ARCHITECTURE_CONTRACT` | Upstream manual-lane packages; `B-ARTIFACT-BODY-01` (manual artifact-body persistence contract); PRD/OpenAPI/event contracts; UI contract |
