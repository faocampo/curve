...
| Source | [Curve PRD v0.13](../curve-ai-native-sdlc-prd.md) (product requirements, approved Product core, Curve-first shell invariant, lifecycle, security invariants, acceptance criteria, and accepted D-003 local implementation) |
...
| M0-09 | M | Provider integration foundation, decomposed into M0-S9A local substrate, M0-S9B external transport/administration, and M0-S9C Model Gateway routing/failover | M0-03, M0-05, M0-07 | FR-003, FR-023, FR-044, NFR-005, NFR-008, NFR-013, AC-33, AC-57 | M0-S9A is accepted locally. [M0-S9B task packet](m0-s9b-provider-transport-task-packet.md) (decision-gated administration, credentials, callbacks, webhooks, schedules, and provider activation) owns completion of AC-33. [M0-S9C task packet](m0-s9c-model-gateway-task-packet.md) (decision-gated model policy, OpenRouter transport, usage, failover, and actual-route evidence) owns AC-57. Both remain blocked until their named decisions and exact child contracts are approved. |
| M0-S9A | M | Workspace-scoped ProviderConnection/ProviderCapability persistence, typed adapter registry, common error taxonomy, deterministic fake adapter, synchronous reconciliation, and explicit bounded local outbox/inbox delivery | M0-03, M0-05, M0-07; P0-05; Option B registration authority; canonical context authorization; exact-head review and merge | FR-003, FR-023, FR-044, NFR-005, NFR-008, NFR-013, partial AC-33 | `DONE_LOCAL`; [M0-S9A task packet](m0-s9a-provider-registry-task-packet.md) (authorization/drain ordering, pending replay, ORM guards, claim exhaustion, stale-result settlement, versioned event payloads, tests, stop conditions, and rollback) and [M0-S9A implementation evidence](m0-s9a-implementation-evidence.md) (exact Plane implementation, CI, regression, dependency disposition, security, and rollback) bind acceptance at Plane `preview` `af7187d...`; the local scope excludes Temporal workflow, Celery task, scheduler, background loop, network, credential, callback, webhook, model call, and public administration. |
| M0-S9B | M, decomposed | Human administration/role source, credential/endpoint profiles, authenticated callback ingress, outgoing webhooks, scheduled reconciliation, and provider activation packets | M0-S9A and each child/provider's applicable decided identity/security/data/infrastructure/external-effect record; D-007 only for MCP/Orca | FR-003, FR-023, FR-044, NFR-005, NFR-008, NFR-013, completes AC-33 | `PREPARED / BLOCKED`; the [M0-S9B task packet](m0-s9b-provider-transport-task-packet.md) (six independently reviewable external-transport children, exact security invariants, acceptance, commands, and rollback) prohibits parent-wide dispatch and requires child-specific approval. |
| M0-S9C | M, decomposed | Immutable model policy, budget reservation, provider-neutral ModelGateway, OpenRouter transport, actual-route usage evidence, failover, and reconciliation | M0-S9A; D-004, D-005, D-014; D-009 before protected model bodies | AC-57 plus the model-enabled portions of FR-002, FR-005, FR-010, FR-017 and NFR-006-NFR-008, NFR-017 | `PREPARED / BLOCKED`; the [M0-S9C task packet](m0-s9c-model-gateway-task-packet.md) (contracts, policy/budget kernel, OpenRouter transport, failover, conformance, commands, and rollback) requires exact model/provider/data/budget decisions and forbids provider-default or silent routing. |

The last verified Plane implementation base is `preview`
`af7187d049c6ee6d0c82a5c70b686d4c444e9b63`, which includes accepted M0-S9A
...
| ID | Size | Deliverable | Dependencies | PRD trace | Completion evidence |
| -- | ---- | ----------- | ------------ | --------- | ------------------- |
| M1-00A | S | Minimal Product aggregate and APIs for the approved immutable key, mutable metadata, prospective IANA timezone, one human owner, ACTIVE/ARCHIVED lifecycle, archive guard, concurrency, and audit/outbox events; roadmap behavior remains in M2 | M0-02, M0-03, M0-07 | FR-025, AC-35, AC-52; P-01-P-18 in the M1-00A task packet | `DONE_LOCAL`: Plane PR #13 merged accepted head `d4ab9ea...` as `afdb593...`; workspace isolation, authority, uniqueness, timezone history, lifecycle guard, concurrency, migration, atomic audit/outbox, API, 355-backend-test, `pnpm check`, `pnpm build`, and commit-bound CI evidence passed. |
| M1-01 | M | Parent work package split into M1-01A (STANDALONE Initiative domain/API with active Product, case-insensitive keyword, risk, creator, three assignments, DRAFT/ALIGNING/PAUSED/CANCELLED lifecycle, and manual-first workflow pin) and M1-01B (Curve-first Initiative shell) | M1-00A, M0-02, M0-03, M0-06-M0-07; M1-01B also requires M1-01A and an approved screen contract | FR-001, FR-042-FR-043, AC-01-AC-02, AC-20; I-01-I-16 in the M1-01A task packet | `IN_PROGRESS`: M1-01A is `DONE_LOCAL`; Plane PR #14 merged reviewed head `7e712e0...` as `99a73b4...`, with 364 passing Curve backend tests, migration proof, monorepo checks/build, and green commit-bound CI recorded in [M1-01A implementation evidence](m1-01a-initiative-core-implementation-evidence.md) (exact context, implementation, tests, merge tree, data boundary, and rollback). M1-01B remains open for browser, accessibility, safe-error, responsive-layout, and manual UX acceptance. |
...
| ID | Size | Deliverable | Dependencies | PRD trace | Completion evidence |
| -- | ---- | ----------- | ------------ | --------- | ------------------- |
| M2-01 | M | Roadmap, Milestone, Feature, Roadmap Item, schedule, and movement-history aggregates extending the M1-00A Product core | M1-00A, M0-02-M0-03, D-013 | FR-025-FR-027, AC-02, AC-37, AC-40 | Product-reference, invariant, same-Feature/multi-Milestone, schedule, and movement audit tests. |
