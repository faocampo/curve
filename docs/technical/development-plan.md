...
| RUNTIME-M0-01 (graceful Curve worker shutdown classification) | S | Correct intentional `SIGINT`/`SIGTERM` classification, preserve independent worker/relay failure propagation, prove bounded cleanup, and retain the existing Temporal SDK and Compose topology | Accepted local M0-06 (Temporal workflow skeleton); D-003 (runtime topology and trust-zone decision); owner approval of the proposed human-operated bootstrap or another complete B-CODING-TOOLS-01/B-CODING-AUTHORITY-01 pair | FR-015, FR-022, NFR-004-NFR-005, enabling portion of AC-58 | `DEFINITION_PREPARED / MANUAL_BOOTSTRAP_PROPOSED / OWNER_DECISION_REQUIRED / NOT_IMPLEMENTATION_AUTHORITY`: [RUNTIME-M0-01 task packet](runtime-m0-01-graceful-shutdown-task-packet.md) (exact Plane base, supervisor behavior, worker-owned quiescence, deterministic lifecycle tests, live signal proof, stop conditions, and rollback) fixes only worker lifecycle classification. [Local execution and authority decision packet](coding-agent-local-execution-decision.md) (verified local feasibility, machine-tool alternatives, proposed human-operated bootstrap, and production fail-closed boundary) awaits Federico's selection. R1-03 (full disaster-recovery exercise) retains ownership of complete AC-58 qualification. |
