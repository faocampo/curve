...
| RUNTIME-M0-01 (graceful Curve worker shutdown classification) | S | Correct intentional `SIGINT`/`SIGTERM` classification, preserve independent worker/relay failure propagation, prove bounded cleanup, and retain the existing Temporal SDK and Compose topology | Accepted local M0-06 (Temporal workflow skeleton); D-003 (runtime topology and trust-zone decision); B-CODING-TOOLS-01 (local coding-tool execution profile) before machine `READY`; B-CODING-AUTHORITY-01 (trusted human authority and attempt lease) before dispatch | FR-015, FR-022, NFR-004-NFR-005, enabling portion of AC-58 | `DEFINITION_PREPARED / MACHINE_READY_TOOL_BLOCKED / DISPATCH_AUTHORITY_REQUIRED / NOT_IMPLEMENTATION_AUTHORITY`: [RUNTIME-M0-01 task packet](runtime-m0-01-graceful-shutdown-task-packet.md) (exact Plane base, supervisor behavior, deterministic lifecycle tests, live signal proof, stop conditions, and rollback) fixes only worker lifecycle classification. [Local execution and authority decision packet](coding-agent-local-execution-decision.md) (tool/trust profiles and human-grant/lease alternatives) must be decided and implemented at the applicable boundary. R1-03 (full disaster-recovery exercise) retains ownership of complete AC-58 qualification. |
