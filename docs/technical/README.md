...
| [M0-S4 foundation probe experience](ux-m0-s4-foundation-probe.md) (clickable local foundation-status flow, state contract, and review script) | UX-004/UX-005 proposal for the Operation API, resumable SSE, cancellation, recovery, and minimal workspace UI | `PROPOSED_REVIEW_REQUIRED`; user-facing M0-S4 implementation remains blocked pending Federico's exact-revision review |
