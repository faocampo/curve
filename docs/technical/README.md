...
and reconciliation foundation) is prepared as a local synthetic review draft
and keeps D-007 (MCP trust-model decision) scoped to MCP/Orca activation. An
...
| [M0-S9A provider-registry task packet](m0-s9a-provider-registry-task-packet.md) (local provider persistence, adapter port, fake reconciliation, acceptance, stop conditions, and rollback) | First independently reviewable child of M0-09; excludes public administration, credentials, callbacks/webhooks, schedules, and real adapters | `REVIEW_DRAFT / NOT_DISPATCHABLE`; Plane PR #9 and P0-05 are satisfied; contract publication, merged context, and exact-head dispatch authorization remain; D-007 is MCP/Orca-specific |
