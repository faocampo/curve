...
- OBS-BIND-001 (local Example Organization OTLP/Prometheus/Grafana binding).
...
| [RUNTIME-M0-01 task packet](runtime-m0-01-graceful-shutdown-task-packet.md) (graceful Curve worker shutdown classification, deterministic tests, live signal evidence, stop conditions, and rollback) | Accepted bounded Plane correction, retained as its immutable implementation definition | `ACCEPTED_AND_MERGED / LOCAL_ONLY` |
| [Coding-agent local execution and authority decision packet](coding-agent-local-execution-decision.md) (approved human-operated option, deferred machine profile, production fail-closed behavior, and rollback) | Decided B-CODING-TOOLS-01 and B-CODING-AUTHORITY-01 bootstrap boundary | `DECIDED / OPTION 3 HUMAN-OPERATED / MACHINE PROFILE DEFERRED TO M4 / NOT IMPLEMENTATION AUTHORITY` |
| [RUNTIME-M0-01 human execution grant](runtime-m0-01-human-execution-grant.md) (exact Plane base, file scope, Docker/VCS effects, tests, exclusions, validity, and rollback) | Consumed one-attempt human-operated implementation authority | `CONSUMED / IMPLEMENTATION ACCEPTED_AND_MERGED / LOCAL_ONLY` |
...
| [ADR-003: Runtime topology](adr-003-runtime-topology.md) | Local/non-local Temporal and Curve runtime boundary | SDK 1.31.0; shared local `dev_env`; direct loopback ports; private deployment requirements; authenticated non-local clients; M0-S3 proof and rollback |
...
| [D-003 private-platform connectivity amendment](d003-private-platform-connectivity-amendment.md) | Shared local network, private deployment requirements, service identity, security boundary, revised M0-S3 proof, and activation inputs | `EFFECTIVE`; approved head `5e165c...` merged as `aece539...` |
...
| [D-002 Onyx delegation decision packet](d002-onyx-delegated-identity-decision-packet.md) (deployed Onyx auth proof, mechanism options, two-user ACL test, revocation, and decision template) | Evidence-complete process for selecting initiating-user Onyx delegation without a reusable PAT or broad service identity | `PROPOSED`; requires named Security/Identity owner and exact Example Organization Onyx proof |
...
| [M0-S5 telemetry manifest](../../contracts/observability/m0-s5-telemetry-v1.json) (fail-closed exporter, bounded metrics, spans, logs, dashboard, four alerts, and redaction contract) | Normative v1 instrumentation and operational-asset surface | Export defaults to disabled; exporter-failure diagnostics stay process-local; Example Organization endpoint/datasource/alert/path-health binding is supplied separately |
...
| [M1-01B implementation task packet](m1-01b-initiative-shell-implementation-task-packet.md) (typed client, Curve route/navigation, list/detail/create/actions, responsive/accessibility acceptance, exact Plane paths and commands, ordered publication, blockers, and rollback) | Normative frontend definition plus the active human-operated execution binding against Plane `preview` `c516a61...` and GitHub Project item `PVTI_lAHOBNjuQc4BgZzOzg4vNto` | `HUMAN_EXECUTION_GRANTED / MACHINE_DISPATCH_BLOCKED / UX_ACCEPTANCE_REQUIRED_BEFORE_MERGE` |
| [M1-01B execution-grant decision](m1-01b-execution-grant-decision.md) (selected human-operated authority, local Node/pnpm procedure, commit-bound CodeQL, manual UX gate, and production fail-closed boundary) | Resolves the local bootstrap path while leaving B-CODING-TOOLS-01 (machine coding-tool execution profile) deferred to M4 | `DECIDED / HUMAN_OPERATED_OUTSIDE_CURVE_DISPATCH / MACHINE_DISPATCH_BLOCKED` |
| [M1-01B human execution grant](m1-01b-human-execution-grant.md) (exact Plane base, frontend paths, local tools, validation, CI/security evidence, manual UX gate, exclusions, and rollback) | One bounded ordinary-repository implementation attempt for the Initiative shell | `ACTIVE / HUMAN_OPERATED_OUTSIDE_CURVE_DISPATCH / LOCAL_ONLY`; merge waits for Designated reviewer's manual UX acceptance |
| [M1-M7 coding-agent task packets](m1-m7-task-packets.md) (exact M1-M6 package traces plus deferred M7 extension charter) | Milestone package outcomes, material gates, executable evidence, rollback, and deterministic materialization rules | M1-M6 catalog prepared for machine materialization; M7 has no active implementation packet until a separately approved product decision and exact trace exist; Designated reviewer is default owner/reviewer until reassigned |
