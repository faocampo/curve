...
| Field | Value |
| --- | --- |
| Status | Active readiness control; D-001 (Plane upstream foundation decision), D-003 (runtime topology and trust-zone decision), and OBS-BIND-001 (local observability binding) are decided for local scope; M0-S3, M0-S4, and M0-S5A are accepted; M0-S5B awaits exact context publication before dispatch |
| Version | 1.18 |
| Date | 2026-08-22 |
...
| Accepted Plane baseline | Fork `preview` at M0-S5A merge `39920769daf78fce29a10c7f4e4bb8779671b004`; preserves the approved upstream candidate and M0-S1 through M0-S5A in its ancestry |
| Published Curve baseline | Curve PR #19 merged exact M0-S5 contract head `fa6fd677fc41d0bc73a8587e78d33d55a6824429` to `main` at `a23dab99e9afcc9dbfad7f5a3dc8b394ef60e529` |
| Published Plane implementation | Plane PR #7 merged exact M0-S5A head `c258ef12221964dae67286e0f6a6c2dc58b997fe` into fork `preview` at `39920769daf78fce29a10c7f4e4bb8779671b004`; both revisions share Git tree `87db1b646cd473f1c407b8aebbc43c27e62d9f8c` |
...
| [OBS-BIND-001](obs-bind-001-local-observability-binding.md) (local Docker OTLP, Prometheus, Grafana, and path-health binding) | DECIDED for `LOCAL_ONLY`; exact-head publication pending | Curve engineering owner | Federico Ocampo, CTO at X3M | Developer running the local stack | M0-S5B context pin only | Existing `dev_env`; OTLP/gRPC without local TLS/auth; digest-pinned Collector/Prometheus/Grafana; loopback host ports; file provisioning; local-only alerts; disposable volumes; independent scrape health; staging requires a separate decision. |
...
| --- | --- | --- |
| P0-01 Plane inventory | DONE | Repository-level capability/license-boundary proof and reuse/build recommendation are approved under D-001; Plane PR #1 established foundation `549db1a...`. M0-01, M0-S2, M0-03, M0-S3, M0-S4, and M0-S5A advanced current `preview` to `3992076...`. |
...
| M0-08 audit/observability | M0-S5A_DONE / M0-S5B_CONTEXT_PENDING | M0-S5A is accepted at Plane merge `3992076...`; [M0-S5A implementation evidence](m0-s5a-implementation-evidence.md) (exact context, approval, merge, complete dual-mode regression, security controls, and rollback) binds the result. [OBS-BIND-001](obs-bind-001-local-observability-binding.md) (local Docker OTLP, Prometheus, Grafana, and path-health binding) is decided; publish and pin its exact merged context, then dispatch M0-S5B. Keep M0-08 visually `In progress` until live local acceptance passes. |
