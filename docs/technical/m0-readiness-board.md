...
| Field | Value |
| --- | --- |
| Status | Active readiness control; D-001 (Plane upstream foundation decision), D-003 (runtime topology and trust-zone decision), and OBS-BIND-001 (local observability binding) are decided for local scope; M0-S3, M0-S4, M0-S5A, M0-S5B, and M0-S6A are accepted and merged; M0-06 and M0-08 are complete for `LOCAL_ONLY`; the M0-S9A (local provider-registry substrate) six-finding correction is merged and Plane implementation remains paused until an immutable external resumption record binds the canonical context, revalidated Plane base, and Federico authorization |
| Version | 1.28 |
| Date | 2026-08-28 |
...
| Accepted Plane baseline | Fork `preview` at `ad5772c0565c934e64ea90f892be1374819979be`; preserves M0-S1 through M0-S5B, Plane PR #9 (policy timestamp-ordering regression fix), and accepted Plane PR #10 (M0-S6A durable parent/child Temporal orchestration implementation) |
| Published Curve correction baseline | Curve `main` contains `da44d27c3bde73b11640b165d3ddbca8451cd1f6`, the squash merge of [Curve PR #36](https://github.com/faocampo/curve/pull/36) (six-finding M0-S9A contract correction), approved at exact head `737c52c52f6f8f8b5f59ec4c69450b2edcacea8d` with identical Git tree `3596a70feecb6fd72f65e3394d7091141b3bbba8`; accepted M0-S5B evidence remains bound at `590a52ef006fd1d83bef5c76dfdab9ce9080a168` |
| Published Plane implementation | Plane `preview` at `ad5772c0565c934e64ea90f892be1374819979be`; Plane PR #10 (M0-S6A durable parent/child Temporal orchestration implementation) is accepted and merged |
...
| M0-S9A local provider-registry substrate | CORRECTION_MERGED / IMPLEMENTATION_PAUSED | M0-03, M0-05, M0-07, Plane PR #9, Plane PR #10 (M0-S6A durable orchestration implementation), P0-05, local delivery, Option B authorization, and the initial explicit Plane dispatch are satisfied. [Curve PR #36](https://github.com/faocampo/curve/pull/36) (six-finding M0-S9A contract correction) closed authorization/drain ordering, pending replay, ORM bulk guards, abandoned-claim exhaustion, stale-result normalization, and event-payload-schema findings at approved head `737c52c...`, squash commit `da44d27c...`, and identical tree `3596a70f...`; CI passed. [M0-S9A task packet](m0-s9a-provider-registry-task-packet.md) (corrected scope, contracts, tests, stop conditions, and rollback) and [M0-S9A relational contract](../../contracts/database/m0-s9a-provider-registry-contract.md) (corrected transactions, persistence guards, delivery recovery, and replay semantics) define dispatch-time evidence for the canonical context, Plane-base revalidation, and human-authorized resumption. An immutable external resumption record satisfies those requirements without editing or redigesting the contracts. The uncommitted Plane implementation remains paused on `curve/m0-s9a-provider-registry-foundation`; provider migration remains `0005` after policy migration `0004`. D-007 is not a dependency. |
