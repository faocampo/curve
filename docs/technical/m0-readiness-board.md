...
| Published Curve baseline | Curve `main` at `d97cc053a5d0eac7bc2aa9bebe263a245c95894f`, containing accepted P0-05 (test strategy and audit closure) and the M0-S6A (durable parent/child Temporal orchestration) contract; accepted M0-S5B evidence remains bound at `590a52ef006fd1d83bef5c76dfdab9ce9080a168` |
...
| M0-06 Temporal skeleton | IN_PROGRESS / DECOMPOSED | M0-S3 completed the executable round trip. M0-S6A (durable parent/child Temporal orchestration) is accepted and merged through Plane PR #10 at `ad5772c...`, including deterministic parent/child waves, reference-only attempt signals, cancellation, restart recovery, replay, and Continue-As-New. Provider-backed attempt and reconciliation integration remains later M0-06/M0-09 scope. |
| M0-S6A durable orchestration | DONE | [M0-S6A task packet](m0-s6a-durable-orchestration-task-packet.md) (deterministic parent/child orchestration contracts, tests, and rollback) is published at Curve `main` `d97cc053...`. Federico Ocampo approved Plane PR #10 at exact head `af8335c...`; it passed attached Plane CI and squash-merged into `preview` as `ad5772c...`. |
...
| M0-S9A local provider-registry substrate | REVIEW_DRAFT / NOT_DISPATCHABLE | M0-03, M0-05, M0-07, Plane PR #9, Plane PR #10 (M0-S6A durable orchestration implementation), and P0-05 are satisfied. The local event-delivery model is approved: synchronous commands, `CURVE_PROVIDER_LOCAL_V1`, `curve-provider-local-v1`, post-commit/next-command drains, workspace/consumer/event dedupe, batch 10, 30-second lease, five-second retry, three attempts, and `DEAD_LETTER`. Dispatch still requires a decided registration action/target/trusted `PLATFORM_ADMINISTRATOR` source, exact merged [M0-S9A task packet](m0-s9a-provider-registry-task-packet.md) (scope, contracts, tests, stop conditions, and rollback), canonical context digest, and exact-head implementation authorization against Plane `preview` `ad5772c...`. Provider migration is `0005` after policy migration `0004`. D-007 is not a dependency. |
