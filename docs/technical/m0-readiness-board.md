...
| Field | Value |
| --- | --- |
| Status | Active readiness control; D-001 (Plane upstream foundation decision), D-003 (runtime topology and trust-zone decision), and OBS-BIND-001 (local observability binding) are decided for local scope; M0-S3, M0-S4, M0-S5A, M0-S5B, M0-S6A, and M0-S9A are accepted and merged for `LOCAL_ONLY`; M0-06 and M0-08 are complete for `LOCAL_ONLY`; RUNTIME-M0-01 is definition-prepared, machine readiness is tool-profile blocked, and dispatch authority remains required; M0-S9B external transport/administration and M0-S9C Model Gateway are decomposed but blocked on named decisions; the published P0-12/D-009 retention proposal remains unresolved and fail closed |
| Version | 1.38 |
| Date | 2026-08-31 |
...
| Observed Curve contract base | Curve `main` at `cbbb9a7397fd65f100344ceb766ad57e8039aaa1`, the squash merge of [Curve PR #51](https://github.com/faocampo/curve/pull/51) (M0-S9B1 provider-administration contracts and governance definition); observation records the source head and grants no decision, implementation, provider-call, or dispatch authority |
...
| [B-CODING-TOOLS-01](coding-agent-local-execution-decision.md) (local coding-tool execution profile) | PROPOSED / OWNER DECISION REQUIRED | Curve Engineering; Security; Platform | Federico Ocampo required | Curve security engineering | RUNTIME-M0-01 machine readiness | Select one trust-tier/tool profile, then implement its exact command grammar, trusted helper, image/tool pins, adversarial tests, and cleanup. |
| [B-CODING-AUTHORITY-01](coding-agent-local-execution-decision.md) (trusted human authority and attempt lease) | PROPOSED / OWNER DECISION REQUIRED | Curve Engineering; Security; Platform | Federico Ocampo required | Curve security engineering | Every coding-agent implementation dispatch; source/evidence/catalog/registry publication and read-only preflight remain allowed | Select one bootstrap/production authority profile, then implement the applicable identity/receipt verification and attempt-lease controls. Production dispatch remains fail closed. |
...
| [RUNTIME-M0-01](runtime-m0-01-graceful-shutdown-task-packet.md) (graceful Curve worker shutdown classification, deterministic tests, live proof, and rollback) | DEFINITION_PREPARED / MACHINE_READY_TOOL_BLOCKED / DISPATCH_AUTHORITY_REQUIRED / NOT_IMPLEMENTATION_AUTHORITY | The Plane defect and exact base `99a73b4...` are verified. The implementation scope is one side-effect-free supervisor helper, one worker integration, and focused tests. B-CODING-TOOLS-01 must provide a truthful Python/Docker/security execution profile before machine `READY`; B-CODING-AUTHORITY-01 must govern the later implementation grant and lease. No Plane mutation is authorized. |
