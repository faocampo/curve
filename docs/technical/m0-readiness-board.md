...
| D-004 (Model Gateway architecture decision) / D-005 (model/provider data-policy decision) | First model-enabled M1 package | PROPOSED | AI platform, AI governance, security |
...
| M0-S9C Model Gateway routing/failover | PREPARED / BLOCKED | [M0-S9C task packet](m0-s9c-model-gateway-task-packet.md) (immutable model policy, budget, OpenRouter transport, actual-route usage, failover, tests, and rollback) defines four children and owns AC-57. D-004 (Model Gateway architecture decision), D-005 (model/provider data-policy decision), and D-014 (budget-policy decision) remain required; D-009 (retention, backup, legal-hold, tombstone, and erasure decision) applies before protected prompt/response persistence. |
