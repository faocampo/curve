...
| M0-06 Temporal skeleton | IN_PROGRESS / DECOMPOSED | M0-S3 completed the executable round trip. M0-S6A implements deterministic parent/child waves, reference-only attempt signals, cancellation, restart recovery, replay, and Continue-As-New in draft Plane PR #10 at exact head `05d2e17...`; it remains `In review`. Provider-backed attempt and reconciliation integration remains later M0-06/M0-09 scope. |
| M0-S6A durable orchestration | IMPLEMENTED / IN_REVIEW | [M0-S6A task packet](m0-s6a-durable-orchestration-task-packet.md) (deterministic parent/child orchestration contracts, tests, and rollback) is published at Curve `main` `d97cc053...`. Draft Plane PR #10 implements it at `05d2e17...`; exact-head human review and merge evidence remain required. |
