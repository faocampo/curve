...
| Published Curve baseline | Curve `main` at `13cec5e99889c68b885a57a8a98609885b1e27b3`, containing accepted P0-05 (test strategy and audit closure), M0-S6A (durable parent/child Temporal orchestration contract and implementation evidence), M0-S9A (provider-neutral registry and dispatch-readiness reconciliation), P0-03A (later-milestone decision-readiness packets), and the accepted policy-v2 correction; accepted M0-S5B evidence remains bound at `590a52ef006fd1d83bef5c76dfdab9ce9080a168` |
...
| M0-S9A local provider-registry substrate | CONTRACT_CORRECTION_IN_REVIEW / IMPLEMENTATION_PAUSED | M0-03, M0-05, M0-07, Plane PR #9, Plane PR #10 (M0-S6A durable orchestration implementation), P0-05, local delivery, Option B authorization, and the initial explicit Plane dispatch are satisfied. Independent review found six contract gaps: authorization/drain ordering, pending replay, ORM bulk guards, abandoned-claim exhaustion, stale-result normalization, and event-payload schemas. [M0-S9A task packet](m0-s9a-provider-registry-task-packet.md) (corrected scope, contracts, tests, stop conditions, and rollback) and [M0-S9A relational contract](../../contracts/database/m0-s9a-provider-registry-contract.md) (corrected transactions, persistence guards, delivery recovery, and replay semantics) await green CI, exact-head approval, squash merge, replacement canonical context digest, and human-authorized Plane resumption. The unmerged Plane implementation remains paused on `curve/m0-s9a-provider-registry-foundation`; provider migration remains `0005` after policy migration `0004`. D-007 is not a dependency. |
