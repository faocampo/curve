...
| Inspected branch and commit | `preview` at `4ae3a77f665368cf8f6a39e9434c2733551cf9d8` |
...
| Runtime evidence | [Exact-preview local runtime refresh](local-runtime-refresh-evidence.md) (historical `99a73b4...` source checkout, Curve migrations `0001`-`0007`, health, dual-mode behavior, replay, restart recovery, and remaining gates) plus [M0 completion audit](m0-completion-audit.md) (accepted `4ae3a77...` descendant, security evidence, remaining packages, and next sequence) |
...
D-001 is `DECIDED`. The verification run exposed and corrected an i18n type-generator formatting defect, and the approved candidate `d380678912e9b46805ef852d2e05411f1fea6d8b` is preserved as a parent of accepted fork `preview` foundation merge `549db1aea8f3307b337b3686dbb844a87549cd95`. Full repository checks and the approved local deployment smoke passed on the candidate tree. The repository-level community/commercial audit, reuse/build recommendation, license obligations, repository boundary, exact-head dispositions, merge, and foundation are accepted in [ADR-001](adr-001-plane-upstream-foundation.md#approval-record) (Plane upstream, licensing, fork, and upgrade decision). M0-01 subsequently proved the feature-disabled migration, workspace isolation, and rollback behavior in Plane PR #2. M0-S2 added the accepted persistence and transactional delivery kernel through Plane PR #3, M0-03 added the accepted authorization/policy kernel through Plane PR #4, M0-S3 added the accepted local Temporal round trip through Plane PR #5, and M0-S4 added the accepted Operation API, resumable SSE, and Curve-first UI through Plane PR #6 at historical checkpoint `e762fbbd2c1726a2833745add8245a1679c60d88`. Accepted additive descendants then added local observability, durable orchestration, M0-S9A (local provider registry), M1-00A (minimal Product core), and M1-01A (Initiative domain/API) at historical capability checkpoint `99a73b4eab5ee21fd012d7358bc9259252d47f71`; runtime-shutdown and Critical/High dependency-remediation descendants advanced current `preview` to `4ae3a77f665368cf8f6a39e9434c2733551cf9d8`.

## Repository and integration state

| Property | Observation | Consequence |
| -------- | ----------- | ----------- |
| Fork branch | `origin/preview` at current accepted revision `4ae3a77f665368cf8f6a39e9434c2733551cf9d8`; historical D-001 foundation `549db1aea8f3307b337b3686dbb844a87549cd95`, M0-S4 checkpoint `e762fbbd2c1726a2833745add8245a1679c60d88`, and M1-01A checkpoint `99a73b4...` are ancestors | The implementation base includes the approved upstream foundation, accepted local M0 control path through M0-S9A, Product/Initiative foundations, graceful shutdown, and Critical/High security and dependency remediations without rewriting shared history. |
...
| Accepted foundation and current working tree | Foundation `549db1aea8f3307b337b3686dbb844a87549cd95`; current `origin/preview` `4ae3a77f665368cf8f6a39e9434c2733551cf9d8` | The exact foundation remains reproducible; M0-S4, M0-S9A, M1-00A, M1-01A, RUNTIME-M0-01, and security-remediation commits remain identifiable accepted checkpoints in the additive lineage. |
...
The product sponsor authorized the official-upstream baseline and isolated-branch preparation on 2026-08-12. Steps 1-7, the foundation merge/base pin, and the independently reviewed M0-01 implementation proof are complete.
...
The candidate correction was committed, published, reviewed, and accepted under D-001 at exact SHA `d380678912e9b46805ef852d2e05411f1fea6d8b`. Plane PR #1 merged it into `preview` at `549db1aea8f3307b337b3686dbb844a87549cd95`. Plane PR #2 then merged M0-S1 at `7685bbc7cc5e1ab34f11e3912d9e47d31c365a9a`. Plane PR #3 merged M0-S2 approved head `f520075493290389aa54532baec36268c34e2885` as `eff8686a69aa112ea8fda79be0e1316dc1fd97d6`. Plane PR #4 merged M0-03 approved head `a807dd7a3f7b81f13ca815b165fba4f4bc068d9e` as `922dd6de5d5ed5081f35cd88343154022867ccad`. Plane PR #5 merged M0-S3 approved head `7fd231b062dc485b37078979a78ec83618be78d8` as `d99342f589db4eb488695487d3ae3f2c16bf0874`. Plane PR #6 merged M0-S4 approved head `a1748c790a060434928b8ed521692b13b3f9739e` at historical checkpoint `e762fbbd2c1726a2833745add8245a1679c60d88`. Plane PR #12 later established M0-S9A checkpoint `af7187d049c6ee6d0c82a5c70b686d4c444e9b63`; Plane PR #13 added M1-00A as `afdb59388e4ea9b2321d33935000126303fc93b8`; Plane PR #14 added M1-01A as capability checkpoint `99a73b4eab5ee21fd012d7358bc9259252d47f71`; Plane PRs #15, #16, and #18 added graceful shutdown, Critical/High CodeQL remediation, and Critical/High production-dependency remediation, advancing current `preview` to `4ae3a77f665368cf8f6a39e9434c2733551cf9d8`. These commits pin the foundation and accepted additive Curve implementation lineage; Curve product releases use their separate release qualification and tagging contract.
...
- [x] Complete M0-03 provider-neutral policy, workspace-isolation, immutable decision/audit, transaction-bound authorization, migration, and regression proof; Plane PR #4 merged at `922dd6de5d5ed5081f35cd88343154022867ccad`. See [M0-03 implementation evidence](m0-03-implementation-evidence.md) (exact contract, context, security validation, and merge record).
- [x] Complete M0-S3 local Temporal workflow, replay, restart, cancellation, security, migration, and rollback proof; Plane PR #5 merged at `d99342f589db4eb488695487d3ae3f2c16bf0874`. See [M0-S3 implementation evidence](m0-s3-implementation-evidence.md) (exact context, merge, tests, runtime proof, security acceptance, and rollback).
- [x] Complete M0-S4 Operation API, resumable SSE, Curve-first UI, accessibility, security, full-regression, migration-drift, and rollback proof; Plane PR #6 merged at `e762fbbd2c1726a2833745add8245a1679c60d88`. See [M0-S4 implementation evidence](m0-s4-implementation-evidence.md) (exact context, approvals, merges, tests, product/security acceptance, and rollback).
- [x] Complete M0-S9A provider-neutral local registry and reconciliation proof; Plane PR #12 merged at `af7187d049c6ee6d0c82a5c70b686d4c444e9b63`. See [M0-S9A implementation evidence](m0-s9a-implementation-evidence.md) (exact context, implementation, validation, security boundary, and rollback).
- [x] Complete the local functional implementation and verification for M1-00A (minimal Product core); Plane PR #13 (M1-00A Product implementation) merged at `afdb59388e4ea9b2321d33935000126303fc93b8`. Exact relational conformance and production qualification remain open under R-027 (Product timestamp/schema-version contract reconciliation). See [M1-00A implementation evidence](m1-00a-product-core-implementation-evidence.md) (exact context, implementation, tests, merge tree, unresolved variance, and rollback).
- [x] Complete M1-01A (Initiative domain and API foundation); Plane PR #14 (M1-01A Initiative backend implementation) merged at `99a73b4eab5ee21fd012d7358bc9259252d47f71`. See [M1-01A implementation evidence](m1-01a-initiative-core-implementation-evidence.md) (exact context, implementation, tests, merge tree, and rollback).
