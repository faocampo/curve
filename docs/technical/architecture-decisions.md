...
| D-002 | PROPOSED | [Onyx per-operation delegated identity](d002-onyx-delegated-identity-decision-packet.md) (deployed auth/OpenAPI proof, grant options, two-user ACL test, revocation, and decision template) | Live/protected M1 knowledge retrieval | Named Security/Identity owner; exact deployed Onyx version/OpenAPI; supported exchange/proxy/session/ephemeral grant; issuer/audience/subject/scopes; lifetime/revocation; two-user source ACL; durable-wait reauthorization; credential-leakage scan; audit; threat model; cleanup and approval. |
...
| D-009 | OPEN / MACHINE PROPOSAL PREPARED | [Retention, deletion, backup, and legal hold](adr-009-retention-and-erasure.md) (asset policy, controlled copies, hold/erasure state machine, and fail-closed behavior) | M0-04 protected storage; protected M1/M4/M6 capabilities; every staging/production activation | [P0-12 decision packet](p0-12-retention-decision-task-packet.md) (owner inputs, machine worksheet, approval protocol, tests, and fail-closed handoff) is ready for review. Named owners must decide 39 policy cells, eight controlled-copy entries, key/hold/restore/external-copy/failure/cost blocks, and six digest-bound approvals. |
