...
| ------ | ------ | ----------- | ------ | -------------------------------- |
| D-001 | DECIDED | [Plane upstream foundation and reuse-versus-build boundary](adr-001-plane-upstream-foundation.md) (approved fork, licensing, reuse, and upgrade strategy) | P0/M0 | Designated reviewer approved ADR digest `0c780a...`, both exact foundation PR heads, the reuse/license boundary, and the repository/upgrade strategy. Plane foundation `549db1a...` and Curve governance baseline `fe8664a...` are merged. Accepted M0 packages through M0-S9A established checkpoint `af7187d...`; M1-00A and M1-01A established historical capability checkpoint `99a73b4...`; accepted runtime and security descendants advanced Plane `preview` to `4ae3a77f665368cf8f6a39e9434c2733551cf9d8`. |
...
| D-003 | DECIDED and implemented for `LOCAL_ONLY`; environment activation inputs OPEN | [Development/staging/production topology and trust zones](adr-003-runtime-topology.md) (runtime topology, Temporal pins, connectivity, and activation boundary) | M0-S3 (local Temporal round trip)/M0-06 (Temporal workflow skeleton); non-local M0/M4/M6 activation | Public reference: local synthetic proof and authenticated non-local clients. Concrete connectivity, platform, persistence, identity, secret storage and operational evidence are private deployment-profile inputs. Environment activation remains blocked until the applicable D-003 and D-009 gates pass. |
| D-004 | `PROPOSED / UNSELECTED / NO_RUNTIME_AUTHORITY` | [Model Gateway architecture ADR](adr-004-model-gateway-architecture.md) (gateway boundary, transport, routing, streaming, reconciliation, deployment, telemetry, replacement, and evidence alternatives) plus [machine proposal](../../contracts/governance/d004-model-gateway-architecture-v1.json) (closed option catalog, port surface, owners, approvals, dependencies, and fail-closed activation) | Model-enabled M1/M3/M5 | M0-S9C1A (candidate Model Gateway architecture and data-policy contracts) prepares decision inputs only. Named owners, actual account/transport/operations evidence, exact selections, digest-bound approvals, and zero computed gaps remain required. |
| D-005 | `PROPOSED / UNSELECTED / EMPTY_ALLOWLIST` | [Model/provider data-policy ADR](adr-005-model-provider-data-policy.md) (classification, RESTRICTED, fallback, evaluation, terms, exception, telemetry, and catalog-drift alternatives) plus [machine proposal](../../contracts/governance/d005-model-data-policy-v1.json) (closed option catalog and raw-byte-bound candidate contract set) | M1/M3/M5 | M0-S9C1A prepares empty fail-closed model catalog, task policy, fallback-equivalence, and RESTRICTED-route evidence candidates. Exact routes, evaluation/red-team/terms evidence, D-004/D-009/D-014 bindings, owners, selections, approvals, and zero computed gaps remain required. |
...
| D-010 | PROPOSED | Example Organization quality, security, and license baseline | M5 | Licensed tool selection, pinned images/rules, normalized thresholds, non-waivable matrix, suppressions, false-positive baseline, prohibited licenses, update owner. |
| D-011 | OPEN | OpenFeature backend and conventions | M5 | Existing Example Organization backend inventory, OpenFeature support, environments, audit, targeting/rollout, HA, license, operations proof; Example frontend Flipt profile. |
...
| D-015 | PROPOSED | Example feature delivery pilot and comparison | R0A/R0B | Approval of Example backend external-contract/staging boundary, authoritative OpenAPI/deployment evidence, reversible Example frontend scope, user approvals, data class, metric definitions, support and rollback commitment. |
