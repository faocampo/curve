...
| Field | Value |
| ----- | ----- |
| Status | Decision index; D-001 (Plane upstream foundation decision) and the D-003 (runtime topology and trust-zone decision) `LOCAL_ONLY` scope are decided; remaining decision scopes require owner approval and applicable proof |
| Version | 0.8 |
| Source | [Curve PRD v0.9 decision register](../curve-ai-native-sdlc-prd.md#decision-register) (controlled product and architecture decisions) |
| Audience | Decision owners, architects, security, operations, product, licensing, and AI planning agents |

## Purpose

The PRD leaves D-001 through D-016 (controlled architecture and product decisions) to named X3M owners and records planning-session directions as `PROPOSED` where applicable. Federico Ocampo decided D-001 (Plane upstream foundation decision) on 2026-08-15 after accepting its engineering, community/commercial boundary, licensing, ownership, and exact-head evidence. On 2026-08-18 he decided D-003 (runtime topology and trust-zone decision) for `LOCAL_ONLY`, approved Temporal Python SDK 1.31.0 and the two-network Compose overlay, retired P0-06A (isolated Temporal feasibility proof) and P0-06B (least-privilege Plane integration proof), and selected M0-S3 (local Temporal round-trip implementation packet) as the executable proof. This document defines how an architecture agent researches and proves proposals, and how an approved decision becomes an implementation prerequisite.
...
| D-003 | DECIDED for `LOCAL_ONLY`; non-local scopes OPEN | [Development/staging/production topology and trust zones](adr-003-runtime-topology.md) (runtime topology, Temporal pins, networks, and environment scope) | M0-S3 (local Temporal round trip)/M0-06 (Temporal workflow skeleton); non-local M0/M4/M6 | Federico Ocampo approved exact head `7826f403...`, Temporal Python SDK 1.31.0, and the two-network Compose overlay. M0-S3 is the executable local proof. P0-06A/P0-06B are superseded standalone gates. Staging/production capacity, residency, HA, RPO/RTO, Kubernetes/gVisor, persistence, secrets, backup/restore, cost, and ownership remain unresolved and fail closed. |
