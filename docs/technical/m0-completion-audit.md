...
| Field | Value |
| --- | --- |
| Status | `AUDITED / M0 INCOMPLETE / CORRECTION_MERGED / IMPLEMENTATION_PAUSED / NOT_IMPLEMENTATION_AUTHORITY` |
| Version | 1.2 |
| Date | 2026-08-28 |
| Product | Curve |
...
| Audited Curve correction baseline | `da44d27c3bde73b11640b165d3ddbca8451cd1f6`, the squash merge of Curve PR #36 (six-finding M0-S9A contract correction) |
...
2. M0-S9A (local provider-registry substrate) has merged initial contracts, an
   explicit Plane dispatch, and the six-finding correction in Curve PR #36.
   Plane implementation remains paused until an immutable external resumption
   record binds the canonical context digest from the exact merged Curve
   `origin/main` revision selected for resumed dispatch, the revalidated Plane
   base, authorized scope and budget, and Federico authorization. Implementation
   review and acceptance evidence then remain required.
...
| M0-S9A (local provider-registry substrate) | Workspace-scoped connection/capability persistence, typed registry, fake adapter, synchronous reconciliation, bounded local delivery, and Option B authorization | Initial contract merged in [Curve PR #29](https://github.com/faocampo/curve/pull/29) (provider-registry contract); [Curve PR #31](https://github.com/faocampo/curve/pull/31) (dispatch-readiness reconciliation), [Curve PR #35](https://github.com/faocampo/curve/pull/35) (policy-v2 correction), and [Curve PR #36](https://github.com/faocampo/curve/pull/36) (six-finding contract correction) are merged. Plane work was explicitly dispatched on `curve/m0-s9a-provider-registry-foundation` and remains paused before commit/push. | `CORRECTION_MERGED / IMPLEMENTATION_PAUSED` | Create the immutable external resumption record binding the canonical digest from the exact merged Curve revision selected for resumed dispatch, revalidated Plane base, scope, budget, and Federico authorization; then finish implementation/tests, review, merge, and publish evidence. |
...
1. Generate the canonical M0-S9A context from the exact merged Curve
   `origin/main` revision selected for resumed dispatch; a
   digest from an open PR remains non-canonical.
2. Reverify the Plane `preview` base. The last verified expected base is
   `ad5772c0565c934e64ea90f892be1374819979be`, and provider migration `0005`
   follows migration `0004`.
3. Obtain explicit authorization to resume the existing Plane dispatch,
   binding that exact merged Curve revision, canonical context digest, and
   still-current Plane base.
4. Complete and verify M0-S9A (local provider-registry substrate) without
   Temporal, Celery, scheduler, background loop, network, credentials,
   callback/webhook, model, or public-administration behavior.
5. Publish exact implementation evidence and keep M0-09 (provider integration
