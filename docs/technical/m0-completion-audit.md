...
| Merged Curve baseline | `72934b9b96420aa0df4ae7c0d5e1f85522003a18` |
...
2. M0-S9A (local provider-registry substrate) has merged contracts and
   dispatch-readiness reconciliation but still requires a canonical context
   digest, explicit Plane dispatch, implementation, and acceptance evidence.
...
| M0-S9A (local provider-registry substrate) | Workspace-scoped connection/capability persistence, typed registry, fake adapter, synchronous reconciliation, bounded local delivery, and Option B authorization | Contract merged in [Curve PR #29](https://github.com/faocampo/curve/pull/29) (provider-registry contract); [Curve PR #31](https://github.com/faocampo/curve/pull/31) (dispatch-readiness reconciliation) is canonical; Plane migration `0005` and implementation are absent | `READY / NOT_DISPATCHED` | Regenerate canonical context, revalidate Plane base, obtain explicit dispatch, implement, test, review, merge, and publish evidence. |
...
1. Review and merge this M0-06 (local Temporal skeleton) closure and M0
   completion audit after exact-head approval and green CI.
2. Regenerate the canonical M0-S9A (local provider-registry substrate) context
   from merged Curve `main`; a digest from an open PR remains non-canonical.
...
4. Obtain explicit Plane dispatch naming the canonical Curve revision/digest,
   exact Plane base, branch, owner/reviewer, scope, commands, budget, data,
   prohibited effects, and rollback.
5. Implement and verify M0-S9A (local provider-registry substrate) without
