...
| Core policy manifest | [`policy/core-policy-v1.json`](policy/core-policy-v1.json) (immutable v1 action allowlist and deny precedence) | Default-deny roles, classifications, environments, ACL/assignment requirements, target policy, and safe projections. |
...
| Policy evaluation input | [`schemas/policy-evaluation.schema.json`](schemas/policy-evaluation.schema.json) (safe subject, workspace, role, ACL, assignment, classification, target, and service context) | Protected bodies, tokens, credentials, and caller-supplied authority fields are excluded. |
| Immutable policy decision | [`schemas/policy-decision.schema.json`](schemas/policy-decision.schema.json) (effect, ordered reasons, manifest/input digests, classification, and safe projection) | `DENY` and human-confirmation results expose no projection. |
